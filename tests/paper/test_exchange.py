...
@pytest.mark.asyncio()
async def test_buy_fills_immediately(paper_client: PaperExchangeClient) -> None:
    ...
    # Balance: 10000 - (0.1 * 50000 * 1.001) = 10000 - 5005 = 4995
    expected = Decimal("10000") - Decimal("0.1") * Decimal("50000") * Decimal("1.001")
    assert paper_client.balance_usdt == expected


@pytest.mark.asyncio()
async def test_sell_fills_immediately(paper_client: PaperExchangeClient) -> None:
    result = await paper_client.create_order(
        symbol="BTCUSDT",
        side=OrderSide.SELL,
        order_type=OrderType.LIMIT,
    ...
    # Balance: 10000 + (0.1 * 50000 * 0.999) = 10000 + 4995 = 14995
    expected = Decimal("10000") + Decimal("0.1") * Decimal("50000") * Decimal("0.999")
    assert paper_client.balance_usdt == expected
...
async def test_protective_order_stored_as_new(
    ...
    # Balance unchanged for protective orders
...
async def test_get_open_orders(paper_client: PaperExchangeClient) -> None:
    # Create a filled order (LIMIT) and a pending protective order
    await paper_client.create_order(
        symbol="BTCUSDT",
        side=OrderSide.BUY,
        order_type=OrderType.LIMIT,
...
@pytest.mark.asyncio()
async def test_commission_calculation(paper_client: PaperExchangeClient) -> None:
    order = await paper_client.create_order(
        symbol="BTCUSDT",
        side=OrderSide.BUY,
        order_type=OrderType.LIMIT,
    ...
    fill = order.fills[0]
    expected_commission = Decimal("0.1") * Decimal("10000") * Decimal("0.001")
    assert fill.commission == expected_commission
    assert fill.commission_asset == "USDT"


@pytest.mark.asyncio()
async def test_balance_tracks_across_trades(
    paper_client: PaperExchangeClient,
) -> None:
    # BUY: cost = 0.1 * 50000 * 1.001 = 5005
    await paper_client.create_order(
        symbol="BTCUSDT",
        side=OrderSide.BUY,
        order_type=OrderType.LIMIT,
    ...
    # SELL: proceeds = 0.1 * 51000 * 0.999 = 5094.9
    await paper_client.create_order(
        symbol="BTCUSDT",
        side=OrderSide.SELL,
        order_type=OrderType.LIMIT,
    ...
    # Net P&L after commissions should be positive
    assert after_sell < Decimal("10000") + Decimal("100")
